...
fn map(p: vec3f) -> vec2f {
    let sphere1_dist = sdf_sphere(p - vec3f(cos(uniforms.time * 2.0)*1.5, 0.0, 0.0), 1.0);
    let sphere2_dist = sdf_sphere(p - vec3f(-cos(uniforms.time * 2.0)*1.5, 0.0, 0.0), 1.0);
    
    let smooth_dist = smin(sphere1_dist, sphere2_dist, 0.5);

    return vec2f(smooth_dist, 1.0);
}
