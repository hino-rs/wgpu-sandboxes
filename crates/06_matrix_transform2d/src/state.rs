...
impl State {
        ...
        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            ...
            entries: &[
                wgpu::BindGroupLayoutEntry {
                    binding: 0,
                    visibility: wgpu::ShaderStages::VERTEX, // Vertexで使用
    ...
    pub fn update(&mut self) {
        // 時間をすすめ、2*PI(TAU)でラッピングして精度低下を防ぐ
        self.time = (self.time + 0.01) % std::f32::consts::TAU;

        // 拡大縮小: 0.5 ~ 1.5 の範囲で往復
        let scale = 1.0 + self.time.sin() * 0.5;
        // 回転角: 時間経過で回転
        let rotation = self.time;
        // 平行移動: 円を描くように移動
        let translation_x = self.time.cos() * 0.3;
        let translation_y = self.time.sin() * 0.3;

        // glamを使ってTRS（平行移動 * 回転 * 拡大縮小）の4x4行列を作成
        let matrix = glam::Mat4::from_scale_rotation_translation(
            glam::Vec3::new(scale, scale, 1.0),
            glam::Quat::from_rotation_z(rotation),
            glam::Vec3::new(translation_x, translation_y, 0.0),
        );

        let uniform_data = Uniform { matrix };
